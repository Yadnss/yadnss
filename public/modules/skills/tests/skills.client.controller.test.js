...
(function() {
	...
	describe('Skills Controller Tests', function() {
		...
		it('$scope.create() with valid form data should send a POST request with the form input values and then locate to new object URL', inject(function(Skills) {
			...
			expect(scope.required_skills).toEqual([]);
			expect(scope.required_sp).toEqual([0,0,0]);
		...
		it('$scope.update() should update a valid Skill', inject(function(Skills) {
			...
			var sampleSkillPutData = new Skills({
				_id: '525cf20451979dea2c000001',
				name: 'New Skill',
				job: { _id: '525cf20451979dea2c000002' },
				required_skills: []
			});
		...
		it('$scope.addRequisite() should add a blank required skill to a collection', function() {
			var reqs = [];
			scope.addRequisite(reqs);
			expect(reqs).toEqual([{ skill: { _id: null }, level: 1 }]);
		});

		it('$scope.removeRequisite() should remove a required skill from the collection', function() {
			var req = {};
			var reqs = [req];
			scope.removeRequisite(reqs, req);
			expect(reqs).toEqual([]);
		});

		it('$scope.mapSkill() replace skill objects with their id', function() {
			var _id = '525a8422f6d0f87f0e407a33',
				req = {skill: {_id: _id}, level: 1};
			expect(scope.mapSkill(req)).toEqual({skill: _id, level: 1});
		});

		it('$scope.hasPermission() should deny users and allow admins', function() {
			var user = {roles: ['user']},
